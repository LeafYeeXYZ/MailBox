...
function EmailPreview({ mail, onClick, onDelete }: { mail: Mail, onClick: (_id: string) => void, onDelete: (_id: string) => Promise<void> }) {
  ...
  return (
    <div className='flex flex-col w-full md:w-[48.5%] xl:w-[32.2%] p-4 relative shadow-sm border rounded-xl bg-gray-50 cursor-pointer transition-all hover:scale-[99%] md:hover:scale-[98%] dark:bg-gray-950 dark:border-black' onClick={() => onClick(mail._id)}>
      ...
      <div className='text-xs my-2 text-gray-500 dark:text-gray-400 overflow-hidden overflow-ellipsis whitespace-nowrap'>来自 {mail.fromName?.length ? `${mail.fromName} <${mail.from}>` : mail.from}</div>
