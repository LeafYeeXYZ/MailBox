...
import { Button, Input, Form, message, Space, ConfigProvider, ThemeConfig } from 'antd'
  ...
  return (
    ...
    >
      <div className='flex items-start justify-center h-full w-full overflow-x-hidden overflow-y-auto'>
        ...
        > 
          {/* 头像, 用户名, 邮箱 */}
          <div className='relative w-full h-32 my-8 border rounded-lg py-[0.95rem] pl-5 bg-gray-50 shadow-sm dark:bg-gray-950 dark:border-black'>
            ...
            <div className='dark:text-white absolute w-1/2 top-[2.4rem] left-[9.1rem] text-lg font-bold text-gray-700 overflow-hidden text-ellipsis text-nowrap'>
              { user?.username ?? '加载中...' }
            </div>
            <div className='absolute w-1/2 top-[4.3rem] left-[9.2rem] text-sm font-bold text-gray-500 dark:text-gray-400 overflow-hidden text-ellipsis text-nowrap'>
              { user?.email }
            </div>
          </div>

          {/* 修改个人信息 */}
          <p className='w-full dark:text-white text-left text-sm font-bold text-gray-700 my-2 pl-1'><FileTextOutlined /> 修改个人信息</p>
          ...
          {/* 重置密码 */}
          <p className='w-full dark:text-white text-left text-sm font-bold text-gray-700 my-2 pl-1'><AuditOutlined /> 重置密码</p>
          <Space.Compact className='w-full my-2'>
            <Input 
          ...
          >修改密码</Button>
          {/* 清除缓存 */}
          <p className='w-full dark:text-white text-left text-sm font-bold text-gray-700 my-2 pl-1'><CodeOutlined /> 高级设置</p>
          <Button 
            onClick={async () => {
              flushSync(() => setDisabled(true))
              await clear()
